...
export default function OwnerDocumentsTab({ clientId, ownerName }: OwnerDocumentsTabProps) {
  ...
  const [documentToDelete, setDocumentToDelete] = useState<OwnerDocument | null>(null)
  ...
  const handleDelete = async (doc: OwnerDocument) => {
    try {
      setDeleting(true)
  ...
  // Build tenant label with unit info
  ...
  const selectedTenantLabel = tenants.find((t) => t.id === selectedTenant)
    ? getTenantLabel(tenants.find((t) => t.id === selectedTenant)!)
    : 'Select tenant *'
    ...
    <div className="flex flex-col flex-1 min-h-0 gap-4 animate-fade-up">
      <div>
        ...
        <p className={`text-base mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
          Upload contracts and documents, assign them to tenants
        </p>
      </div>
